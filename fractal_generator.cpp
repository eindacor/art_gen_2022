	...
	int num_points)
{
	...
	max_point_size = range[1] < 1 ? std::min(context->getWindowHeight(), context->getWindowWidth()) : std::min(std::min(context->getWindowHeight(), context->getWindowWidth()), range[1]);
...
void fractal_generator::drawFractal(std::weak_ptr<ogl_camera_flying> camera) const
{
	// bind target VAO
	glBindVertexArray(VAO);
	glEnableVertexAttribArray(0);
	...
	if (dof_enabled) {
		...
		vec3 camera_vector = glm::normalize(cam->getFocus() - cam->getPosition());
		...
		for (int i = 0; i < dof_passes; i++)
		{
			...
			glm::mat4 modelview = glm::lookAt(cam->getPosition() + dof_aperture * bokeh, cam->getFocus(), camera_up);
			glm::mat4 mvp = cam->getProjectionMatrix() * modelview;
...
std::vector<std::pair<string, mat4>> fractal_generator::generateMatrixVector(const int &count, geometry_type &geo_type)
{
	...
	if (random_generator::getRandomFloat() < sm.matrix_geometry_coefficient)
	{
		...
		if (matrix_geometry_index < GEOMETRY_TYPE_SIZE) {
			auto gt = geometry_type(matrix_geometry_index);
			...
			auto nt = ngon_type(matrix_geometry_index);
		...
		for (int i = 0; i < count; i++) {
			...
			matrix_vector.emplace_back("translate (" + std::to_string(matrix_geometry_index) + ")", glm::translate(mat4(1.0f), vertex));
		}
	} else {
		...
		for (int i = 0; i < count; i++) {
			...
			matrix_vector.emplace_back(matrix_category, matrix_to_add);
		}
	}
...
void fractal_generator::applyBackground(const int &num_samples)
{
	...
	color_man.adjustLightness(background_color, random_generator::getRandomFloat());
