...
std::uniform_real_distribution<double> random_generator::doubleGenerator(0., 1.);
std::uniform_real_distribution<float> random_generator::floatGenerator(0., 1.);
std::uniform_int_distribution<int> random_generator::intGenerator(0, std::numeric_limits<int>::max());
std::unique_ptr<std::default_random_engine> random_generator::defaultEngine = nullptr;
...
float random_generator::getRandomFloatInRange(const float &min, const float &max)
{
	if (max < min)
		return 0.0f;
	...
	return min + (getRandomFloat() * difference);
}

float random_generator::getRandomUniform()
{
...
unsigned int random_generator::getRandomIntInRange(const unsigned int &min, const unsigned int &max)
{
	if (min == max) {
        return min;
    }
	...
	return min + (intGenerator(getRNG()) % (max - min));
}
...
float random_generator::getRandomFloat() {
    return floatGenerator(getRNG());
}

std::default_random_engine& random_generator::getRNG() {
    if (nullptr == random_generator::defaultEngine) {
        unsigned seed = std::chrono::system_clock::now().time_since_epoch().count();
        defaultEngine = std::make_unique<std::default_random_engine>(std::default_random_engine(seed));
    }

    return random_generator::defaultEngine.get()[0];
}
