...
public:
	random_generator();
	explicit random_generator(const std::string &seed_string);
	...
	static float getRandomFloatOut();
	float getRandomFloatInScope() const { return (getRandomFloat() * 2.0f) - 1.0f; }
	static float getRandomFloatInRange(const float &min, const float &max) ;
	static float getRandomUniform() ;
	glm::vec4 getRandomVec4() const { return {getRandomFloat(), getRandomFloat(), getRandomFloat(), 1.0f}; }
	glm::vec4 getRandomVec4AlphaClamp(float alpha_min, float alpha_max) const { return {getRandomFloat(), getRandomFloat(), getRandomFloat(), getRandomFloatInRange(alpha_min, alpha_max)}; }
	...
	static unsigned int getRandomIntInRange(const unsigned int &min, const unsigned int &max) ;
...
private:
    static std::unique_ptr<std::default_random_engine> defaultEngine;
    static std::uniform_real_distribution<double> doubleGenerator;
    static std::uniform_real_distribution<float> floatGenerator;
    static std::uniform_int_distribution<int> intGenerator;
